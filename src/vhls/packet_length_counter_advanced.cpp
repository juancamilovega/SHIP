...
void packet_length_counter_advanced (
	hls::stream<dataword> data_in,
	hls::stream<dataword> data_out,
	hls::stream<ap_uint<8> > burst_info,
...
#pragma HLS resource core=AXI4Stream variable = burst_info

	static ap_uint<64> current_count=0;
	static ap_uint<8> bursting_count=0;
	dataword temp_data;
	if (!data_in.empty() && !data_out.full()&& !length.full())
	{
		...
		if (temp_data.last==0)
		{
			...
			{
				bursting_count = bursting_count + 1;
			}
		}
		else
		{
			burst_info.write(bursting_count+1);
